...
local function ShowLoadingNotification()
    SendGriefNotify("Ken Hub", "Loading script, please wait", 5)
    return function() end
end
...
local function SafeRequire(moduleRef, timeoutSec)
    ...
    error("[ken hub] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end
...
local function loadIcon(name, svgUrl)
    ...
    if not isfile(path) then
        ...
        if not data then
            warn("[kenHubRivals] Failed to download icon: " .. name)
            return nil
        end
        local written = pcall(writefile, path, data)
        if not written then
            warn("[kenHubRivals] Failed to cache icon: " .. name)
            return nil
        end
    end
    ...
    warn("[kenHubRivals] Failed to load custom asset: " .. name)
    return nil
end
...
local KenHubRedTheme = { FontColor = "ffffff", MainColor = "24171a", AccentColor = "e53935", BackgroundColor = "160d0f", OutlineColor = "3a171b", FontFace = "Code", BackgroundImage = "" }
pcall(function()
    ThemeManager:SetLibrary(Library)
    ThemeManager:SetDefaultTheme(KenHubRedTheme)
end)
GriefWindow = Library:CreateWindow({ Title = "Ken Hub | Rivals", Footer = "Ken Hub | Red", Center = true, AutoShow = true, NotifySide = "Right", ShowCustomCursor = false })
Window = GriefWindow
...
pcall(function()
    if Library and type(Library.AddDraggableLabel) == "function" then
        local wm = Library:AddDraggableLabel("Ken Hub")
...
pcall(function()
    SendGriefNotify("Ken Hub", "Loading script, please wait", 5)
    if Library and Library.Notify then
        Library:Notify({ Title = "Ken Hub", Description = "Loading script, please wait", Time = 5 })
    end
end)
...
if not config.profile.regionspoof then
    config.profile.regionspoof = { enabled = false, value = "ken hub" }
elseif config.profile.regionspoof.value == ".gg/getgrief" then
    config.profile.regionspoof.value = "ken hub"
end
...
regionTab:AddInput("RegionSpoofValue", {
    ...
    Callback = function(val)
        config.profile.regionspoof.value = val or "ken hub"
...
print('[ken hub] main body loaded')